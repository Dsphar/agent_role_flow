...
- can_loop config flag: Line 3 of `loop_state.md` carries `can_loop={true|false}` alongside `test_level` and `skip_docs`. Interviewer initializes it to `false`; Planner sets it to `true` before transitioning. The pipeline-auto extension reads this flag for its guard and proactive footer status.
