...
- Auto-mode softening (Worker through Finalizer): When user is away (pipeline-push mode), roles no longer hang waiting for input at decision points. Worker uses best judgment on ambiguity/blockers; Tester defaults to send-back for in-scope bugs and silently applies skip-docs recommendations (never auto-installs packages); Reviewer absorbs out-of-scope items with TODO creation and defaults to send-back on critical issues; Finalizer logs unexplained changes as TODOs silently. Triggered by user indicating they're away, not the `can_loop` flag.
